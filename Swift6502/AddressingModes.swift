...
let AddressingModeZeroPageX = AddressingMode(
    ...
    instructionSize: 2,
    resolve: { (cpu, mem) in return mem.wordAt(UInt16(mem.byteAt(cpu.PC + 1)) + cpu.X16) }
)

let AddressingModeZeroPageY = AddressingMode(
    name: "AddressingModeZeroPageY",
    instructionSize: 2,
    resolve: { (cpu, mem) in return mem.wordAt(UInt16(mem.byteAt(cpu.PC + 1)) + cpu.Y16) }
)

let AddressingModeAbsoluteX = AddressingMode(
    name: "AddressingModeAbsoluteX",
    instructionSize: 3,
    resolve: { (cpu, mem) in return mem.wordAt(mem.wordAt(cpu.PC + 1) + cpu.X16) }
)

let AddressingModeAbsoluteY = AddressingMode(
    name: "AddressingModeAbsoluteY",
    instructionSize: 3,
    resolve: { (cpu, mem) in return mem.wordAt(mem.wordAt(cpu.PC + 1) + cpu.Y16) }
)
